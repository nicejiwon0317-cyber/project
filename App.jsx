...
function useMusic() {
  ...
  const noteFreq = (note) => {
    ...
    if (!m) return 440;
    const base = map[m[1][0]] + (m[1][1]==='#'?1:0);
    return 440 * Math.pow(2, (base + (parseInt(m[2])-4)*12 - 9) / 12);
  };

  const playNote = useCallback((ctx, freq, when, dur, type="triangle", vol=0.12) => {
    try {
      ...
      g.gain.linearRampToValueAtTime(0.001, when + dur * 0.85);
  ...
  const scheduleMusic = useCallback((mode) => {
    const ctx = getCtx();
    const normal = {
      notes: ["C4","E4","G4","E4","F4","A4","G4","E4","D4","F4","A4","G4","C5","B4","G4","E4"],
      durs:  [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 1.0, 0.5, 0.5, 0.5, 0.5, 0.5, 1.0, 0.5, 0.5, 2.0],
      bpm: 116
    };
    const urgent = {
      notes: ["C5","C5","B4","C5","D5","C5","B4","A4","G4","A4","B4","C5","D5","E5","D5","C5"],
      durs:  [0.25,0.25,0.25,0.25,0.25,0.25,0.25,0.5, 0.25,0.25,0.25,0.25,0.25,0.5, 0.25,0.5],
      bpm: 170
    };
    ...
    seq.notes.forEach((note, i) => {
      const dur = seq.durs[i] * beat;
      playNote(ctx, noteFreq(note), t, dur, mode==="urgent"?"square":"triangle", 0.12);
      if (i % 4 === 0) {
        const bass = ["C3","F3","G3","A3"][Math.floor(i/4)%4];
        playNote(ctx, noteFreq(bass), t, beat*2, "sine", 0.07);
      }
      if (mode === "urgent" && i % 2 === 0) {
        playNote(ctx, 55, t, beat*0.15, "sine", 0.18);
      }
      t += dur;
    });
  ...
  const start = useCallback((mode) => {
    ...
    const loop = () => {
      if (modeRef.current !== mode) return;
      const ms = scheduleMusic(mode);
      loopRef.current = setTimeout(loop, ms - 120);
    };
    loop();
  }, [scheduleMusic]);
  ...
  const playEffect = useCallback((type) => {
    try {
      ...
      if (type === "correct") {
        [[523,0,0.15],[659,0.07,0.15],[784,0.14,0.25]].forEach(([freq,delay,dur]) => {
          playNote(ctx, freq, t+delay, dur, "triangle", 0.18);
        });
      } else if (type === "wrong") {
        [[280,0,0.12],[220,0.1,0.2]].forEach(([freq,delay,dur]) => {
          playNote(ctx, freq, t+delay, dur, "sawtooth", 0.15);
        });
      } else if (type === "timeout") {
        [[440,0,0.08],[440,0.1,0.08],[330,0.2,0.2]].forEach(([freq,delay,dur]) => {
          playNote(ctx, freq, t+delay, dur, "square", 0.14);
        });
      }
    } catch(e) {}
  }, [getCtx, playNote]);
...
async function getRanking() {
  try {
    const res = await fetch(SUPABASE_URL + "/rest/v1/ranking?select=*&order=best.desc&limit=20", {
      headers: SB_HEADERS
    });
    if (!res.ok) return [];
...
async function submitScore(userId, pct) {
  try {
    ...
    const res = await fetch(SUPABASE_URL + "/rest/v1/ranking", {
      ...
      body: JSON.stringify({ user_id: userId, best: pct, tries: 1, last: pct })
    });
    // 이미 있는 유저면 best 점수만 업데이트
    if (!res.ok) {
      const existing = await fetch(SUPABASE_URL + "/rest/v1/ranking?user_id=eq." + encodeURIComponent(userId), {
        headers: SB_HEADERS
      });
      const data = await existing.json();
      if (data && data[0]) {
        const prev = data[0];
        await fetch(SUPABASE_URL + "/rest/v1/ranking?user_id=eq." + encodeURIComponent(userId), {
          method: "PATCH",
          headers: SB_HEADERS,
          body: JSON.stringify({ best: Math.max(prev.best, pct), tries: prev.tries + 1, last: pct })
        });
      }
    }
...
function Bar({ v, t, color1="#5a8a5a", color2="#80c080" }) {
  return (
    <div style={{ height:6, background:"#e8e0d4", borderRadius:3, overflow:"hidden" }}>
      <div style={{ height:"100%", width:(v/t*100)+"%", background:`linear-gradient(90deg,${color1},${color2})`, transition:"width 0.4s ease, background 0.5s" }} />
    </div>
  );
}

function Tag({ label }) {
  const c = { 심리:"#d4a8c7",언어:"#a8c4d4",수사:"#c4d4a8",논리:"#d4c4a8",사회:"#a8d4c8" };
  return <span style={{ background:c[label]||"#e0d8cc", color:"#4a3728", fontSize:11, fontWeight:600, padding:"2px 8px", borderRadius:20 }}>{label}</span>;
}

function Dots({ level }) {
  return <span style={{ display:"flex", gap:3 }}>{[1,2,3].map(i=><span key={i} style={{ width:7,height:7,borderRadius:"50%",background:i<=level?"#8b6914":"#e0d8cc" }}/>)}</span>;
}
...
function Login({ onLogin }) {
  ...
  const go = () => {
    const id = v.trim();
    if (!id) return setErr("닉네임을 입력해주세요.");
    if (id.length < 2 || id.length > 12) return setErr("2~12자로 입력해주세요.");
    onLogin(id);
  };
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#2d1f0e,#4a3020)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
      ...
      <div style={{ width:"100%", maxWidth:320, background:"rgba(255,255,255,0.07)", borderRadius:20, padding:28, border:"1px solid rgba(200,168,130,0.25)" }}>
        ...
        <input value={v} onChange={e=>{setV(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&go()}
          placeholder="닉네임 입력 (2~12자)" maxLength={12}
          style={{ width:"100%", padding:"12px 14px", borderRadius:12, border:"1px solid rgba(200,168,130,0.3)", background:"rgba(255,255,255,0.08)", color:"#faf0e0", fontSize:15, outline:"none", boxSizing:"border-box", fontFamily:"inherit" }} />
...
function Quiz({ words, onDone }) {
  ...
  const build = useCallback((w) => {
    ...
    return s.map(q => {
      const wrongs = w.filter(x=>x.word!==q.word).sort(()=>Math.random()-0.5).slice(0,3);
      const opts = [...wrongs.map(x=>x.word), q.word].sort(()=>Math.random()-0.5);
      return { ...q, options:opts, answer:q.word };
    });
  }, []);
  ...
  const startTimer = useCallback(() => {
    ...
    ivRef.current = setInterval(() => {
      setTimeLeft(t => { if (t <= 1) { clearInterval(ivRef.current); return 0; } return t-1; });
    }, 1000);
  }, []);
  ...
  const handleStart = () => {
    const q = build(words);
    setQs(q); setQi(0); setScore(0); scoreRef.current=0; setChosen(null); setDone(false);
    setStarted(true);
  };

  // 문제 바뀔 때 타이머 + 음악
  useEffect(() => {
    if (!started || !qs.length || done) return;
    startTimer();
    startMusic("normal");
    return () => { if (ivRef.current) clearInterval(ivRef.current); };
  }, [qi, started, qs.length, done]);

  // 시간별 음악 전환
  useEffect(() => {
    if (!started || done || chosen) return;
    if (timeLeft > 0 && timeLeft <= 5) startMusic("urgent");
    else if (timeLeft > 5) startMusic("normal");
  }, [timeLeft, started, done, chosen]);

  // 시간 초과
  useEffect(() => {
    if (!started || timeLeft !== 0 || chosen || !qs.length || done) return;
    playEffect("timeout");
    setChosen("__timeout__");
    if (qi === qs.length - 1) {
      setTimeout(() => { stopMusic(); setDone(true); onDone(scoreRef.current, qs.length); }, 900);
    }
  }, [timeLeft, started, chosen, qs.length, done, qi]);
  ...
  if (!started) return (
    <div style={{ textAlign:"center", padding:"40px 20px" }}>
      ...
      <button onClick={handleStart} style={{ padding:"14px 40px", background:"linear-gradient(135deg,#8b6914,#c8a030)", color:"#fff", border:"none", borderRadius:16, cursor:"pointer", fontWeight:700, fontSize:16, boxShadow:"0 4px 16px rgba(139,105,20,0.4)" }}>
        퀴즈 시작!
      </button>
    </div>
  );
  ...
  const pick = (opt) => {
    ...
    setChosen(opt);
    if (qi === qs.length - 1) {
      setTimeout(() => { stopMusic(); setDone(true); onDone(scoreRef.current, qs.length); }, 800);
    }
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      ...
      <div style={{ height:8, background:"#e8e0d4", borderRadius:4, overflow:"hidden" }}>
        <div style={{ height:"100%", width:(timeLeft/QUESTION_TIME*100)+"%", background:timerBar, transition:"width 1s linear, background 0.5s" }} />
      </div>
      {/* 점수 */}
      <div style={{ display:"flex", justifyContent:"space-between", color:"#7a6650", fontSize:13 }}>
        <span>{qi+1} / {qs.length}</span><span>점수: {score}</span>
      </div>
      {/* 문제 */}
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {q.options.map((opt,i) => {
          const isAns=opt===q.answer, isCh=opt===chosen;
          let bg="#faf6f0", bd="#d4c8b4", cl="#3d2b1a";
          if (chosen) {
            if (isAns) { bg="#e8f4e8"; bd="#5a8a5a"; cl="#2d5a2d"; }
            else if (isCh&&!isTimeout) { bg="#f4e8e8"; bd="#8a5a5a"; cl="#5a2d2d"; }
          }
          return (
            <button key={i} disabled={!!chosen} onClick={()=>pick(opt)}
              style={{ padding:"12px 16px", background:bg, border:"1px solid "+bd, borderRadius:12, cursor:chosen?"default":"pointer", color:cl, fontSize:15, fontFamily:"'Noto Serif KR',serif", fontWeight:600, textAlign:"left", transition:"all 0.2s" }}>
              {String.fromCharCode(65+i)}. {opt}
            </button>
          );
        })}
      </div>
      {chosen && qi < qs.length-1 && (
        <button onClick={()=>{setChosen(null);setQi(x=>x+1);}} style={{ padding:"10px", background:"#8b6914", color:"#fff", border:"none", borderRadius:12, cursor:"pointer", fontWeight:600, fontSize:14 }}>
          다음 문제 →
        </button>
      )}
    </div>
...
function Ranking({ userId }) {
  ...
  return (
    <div>
      ...
      {loading ? (
        <div style={{ textAlign:"center", padding:40, color:"#a09080" }}>불러오는 중...</div>
      ) : list.length === 0 ? (
        <div style={{ textAlign:"center", padding:40, color:"#a09080" }}>
          <div style={{ fontSize:36, marginBottom:8 }}>📊</div>
          아직 기록이 없어요.<br/>퀴즈를 풀면 여기에 등록돼요!
        </div>
      ) : (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {list.map((e,i) => (
            <div key={e.userId} style={{ background:e.userId===userId?"linear-gradient(135deg,#fff8ec,#faf0d8)":"#faf6f0", border:e.userId===userId?"1.5px solid #c8a882":"1px solid #e0d8cc", borderRadius:14, padding:"12px 16px", display:"flex", alignItems:"center", gap:12 }}>
              <div style={{ fontSize:i<3?24:14, width:32, textAlign:"center", fontWeight:700, color:"#a09080" }}>{i<3?MEDAL[i]:i+1}</div>
              <div style={{ flex:1 }}>
                <div style={{ fontWeight:700, color:"#2d1f0e", fontSize:15 }}>{e.userId} {e.userId===userId&&<span style={{ fontSize:11, color:"#8b6914" }}>(나)</span>}</div>
                <div style={{ fontSize:12, color:"#a09080", marginTop:2 }}>도전 {e.tries}회 · 최근 {e.last}점</div>
              </div>
              <div style={{ textAlign:"right" }}>
                <div style={{ fontSize:20, fontWeight:700, color:"#8b6914" }}>{e.best}점</div>
                <div style={{ fontSize:11, color:"#a09080" }}>최고</div>
              </div>
            </div>
          ))}
        </div>
...
export default function App() {
  ...
  const handleDone = async (score, total) => {
    ...
    await submitScore(userId, pct);
  };

  const generate = async () => {
    setGenLoading(true); setGenMsg("AI가 어휘 생성 중...");
    try {
      const c = cat!=="전체"?cat:"";
      const nw = await generateWords(5, c, words);
      setWords(w=>[...w,...nw]);
      setGenMsg("✓ "+nw.length+"개 추가!");
      setTimeout(()=>setGenMsg(""), 2000);
    } catch { setGenMsg("실패. 다시 시도해주세요."); setTimeout(()=>setGenMsg(""),2000); }
    setGenLoading(false);
  };
        ...
        {tab!=="ranking" && (
          <>
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:8 }}>
              {CATEGORIES.map(c=><button key={c} onClick={()=>setCat(c)} style={fBtn(cat===c)}>{c}</button>)}
            </div>
            <div style={{ display:"flex", gap:6, marginBottom:12 }}>
              {LEVELS.map(l=><button key={l} onClick={()=>setLv(l)} style={fBtn(lv===l)}>{l==="전체"?"전체 난이도":"난이도 "+l}</button>)}
            </div>
